...
const rootMap = {
  'index.html': [{name: '首页', url: '/'}],
  'top10.html': [{name: '首页', url: '/'}, {name: 'TOP 10 排名', url: '/top10'}],
  'free-text-to-video.html': [{name: '首页', url: '/'}, {name: '免费文字转视频', url: '/free-text-to-video'}],
  'comparison.html': [{name: '首页', url: '/'}, {name: '工具对比', url: '/comparison'}],
  'tutorial.html': [{name: '首页', url: '/'}, {name: '使用教程', url: '/tutorial'}],
  'free-ai-avatar-video.html': [{name: '首页', url: '/'}, {name: '免费AI数字人视频', url: '/free-ai-avatar-video'}],
  'free-ai-video-translate.html': [{name: '首页', url: '/'}, {name: '免费AI视频翻译', url: '/free-ai-video-translate'}],
  'resources.html': [{name: '首页', url: '/'}, {name: '资源大全', url: '/resources'}],
  'about.html': [{name: '首页', url: '/'}, {name: '关于我们', url: '/about'}],
  'privacy.html': [{name: '首页', url: '/'}, {name: '隐私政策', url: '/privacy'}],
};

const titleMap = {
  'sora-complete-tutorial-2026': 'Sora AI完整使用教程',
  'sora-review-2026': 'Sora 2026年完整评测',
  'sora-alternatives-free': '免费Sora替代工具',
...
function addBreadcrumbJsonLd(html, items) {
  const breadcrumbJson = {
    ...
    'itemListElement': items.map((item, i) => ({
      ...
      'position': i + 1,
      'name': item.name,
      'item': `https://hxqsmj.ccwu.cc${item.url}`
    }))
  };
  const script = `\n<script type="application/ld+json">\n${JSON.stringify(breadcrumbJson, null, 2)}\n</script>`;
  // Insert after existing JSON-LD or before </head>
...
// Root pages
Object.entries(rootMap).forEach(([filename, items]) => {
  const filepath = path.join(__dirname, filename);
  if (!fs.existsSync(filepath)) return;
  let html = fs.readFileSync(filepath, 'utf8');
  if (html.includes('BreadcrumbList')) return; // already added
  html = addBreadcrumbJsonLd(html, items);
...
if (fs.existsSync(articlesDir)) {
  fs.readdirSync(articlesDir).filter(f => f.endsWith('.html')).forEach(filename => {
    ...
    let html = fs.readFileSync(filepath, 'utf8');
    if (html.includes('BreadcrumbList')) return;
    const slug = filename.replace('.html', '');
    const name = titleMap[slug] || slug;
    const items = [
      {name: '首页', url: '/'},
      {name: '文章', url: '/tutorial'},
      {name, url: `/articles/${slug}`}
    ];
    html = addBreadcrumbJsonLd(html, items);
    fs.writeFileSync(filepath, html, 'utf8');
